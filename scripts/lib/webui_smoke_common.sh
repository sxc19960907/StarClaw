...
async function runCore(page) {
  ...
  await page.locator("#council-detail").getByRole("heading", { name: "Council stages" }).waitFor();
  ...
  await page.locator(".council-stage-card").filter({ hasText: "planner" }).getByRole("button", { name: "Copy notes" }).click();
  ...
  await page.locator(".council-stage-card").filter({ hasText: "planner" }).getByRole("button", { name: "Draft to chat" }).click();
  await page.locator("#panel-chat.active").waitFor();
  assert((await page.locator("#chat-input").inputValue()).includes("Role: planner"), "council role draft missing role");
  await page.getByRole("button", { name: "Agent Council" }).click();
  await page.locator("#panel-council.active").waitFor();
  await page.locator("#council-detail").getByText("Synthesis", { exact: true }).waitFor();
  ...
  const finalSynthesisSection = page.locator(".run-detail-section").filter({ has: page.getByRole("heading", { name: "Final synthesis" }) });
  await finalSynthesisSection.getByRole("button", { name: "Copy synthesis" }).click();
  ...
  await page.locator(".council-stage-card").filter({ hasText: "Handoff" }).getByRole("button", { name: "Start run" }).click();
  await page.locator("#panel-runs.active").waitFor();
  ...
  await page.getByRole("heading", { name: "Comparison Workbench" }).waitFor();
  await page.locator(".comparison-lane").nth(2).waitFor();
  await page.locator(".comparison-lane").filter({ hasText: "Runs" }).getByText(/complete|failed|Latest/).first().waitFor();
  await page.locator("#comparison-detail").getByRole("heading", { name: "Evidence", exact: true }).waitFor();
  await page.locator(".comparison-lane").filter({ hasText: "Runs" }).getByRole("button", { name: "Draft compare" }).click();
  await page.locator("#panel-chat.active").waitFor();
  assert((await page.locator("#chat-input").inputValue()).includes("Compare recent Astria runs"), "comparison draft missing run prompt");
  await page.getByRole("button", { name: "Compare" }).click();
  await page.locator("#panel-compare.active").waitFor();
  await page.locator('[data-compare-lane="council-synthesis"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-council.active").waitFor();
  await openManagePanel(page, "Run Quality");
  await page.locator("#panel-quality.active").waitFor();
  await page.locator("#panel-quality").getByRole("heading", { name: "Run Quality" }).waitFor();
  ...
  await page.locator('[data-run-quality="completed-output"]').getByText("Completed output readiness", { exact: true }).waitFor();
  await page.locator('[data-run-quality="failure-retry"]').getByText("Failure and retry risk", { exact: true }).waitFor();
  await page.locator('[data-run-quality="evidence-quality"]').getByText("Evidence quality score", { exact: true }).waitFor();
  await page.locator('[data-run-quality="budget-posture"]').getByText("Budget and stop-rule posture", { exact: true }).waitFor();
  await page.locator('[data-run-quality="reuse-readiness"]').getByText("Reusable output readiness", { exact: true }).waitFor();
  await page.locator('[data-run-quality="delivery-readiness"]').getByText("Delivery readiness score", { exact: true }).waitFor();
  await page.locator("#run-quality-detail").getByRole("heading", { name: "Signal", exact: true }).waitFor();
  await page.locator('[data-run-quality="budget-posture"]').getByRole("button", { name: "Quality brief" }).click();
  await page.locator("#run-quality-detail").getByText("Budget shape", { exact: false }).waitFor();
  await page.locator('[data-run-quality="budget-posture"]').getByRole("button", { name: "Draft review" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Run Quality" }).click();
  await page.locator("#panel-quality.active").waitFor();
  await page.locator('[data-run-quality="budget-posture"]').getByRole("button", { name: "Open route" }).click();
  ...
  await page.locator("#panel-promptlab").getByRole("heading", { name: "Prompt Lab" }).waitFor();
  await page.getByLabel("Prompt lab goal").fill("Ship the next Kocoro parity slice");
  await page.locator(".prompt-variant").nth(3).waitFor();
  await page.locator('[data-prompt-variant="direct"]').getByText("Direct execution").waitFor();
  await page.locator('[data-prompt-variant="evidence"]').getByText("Evidence-first experiment").waitFor();
  await page.locator('[data-prompt-variant="council"]').getByText("Council-reviewed variant").waitFor();
  await page.locator('[data-prompt-variant="delivery"]').getByText("Delivery-ready variant").waitFor();
  await page.locator("#promptlab-detail").getByRole("heading", { name: "Evaluation", exact: true }).waitFor();
  await page.locator('[data-prompt-variant="evidence"]').getByRole("button", { name: "Draft variant" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Prompt Lab" }).click();
  await page.locator("#panel-promptlab.active").waitFor();
  await page.locator('[data-prompt-variant="council"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-council.active").waitFor();
  await openManagePanel(page, "Budget Guard");
  await page.locator("#panel-budget.active").waitFor();
  await page.locator("#panel-budget").getByRole("heading", { name: "Budget Guard" }).waitFor();
  await page.locator(".budget-guard-card").nth(6).waitFor();
  await page.locator('[data-budget-guard="hard-cap"]').getByText("Hard budget cap", { exact: true }).waitFor();
  await page.locator('[data-budget-guard="model-route"]').getByText("Complexity-based model route", { exact: true }).waitFor();
  await page.locator('[data-budget-guard="context-trim"]').getByText("Context trimming pass", { exact: true }).waitFor();
  await page.locator('[data-budget-guard="fallback"]').getByText("Automatic fallback ladder", { exact: true }).waitFor();
  await page.locator('[data-budget-guard="stop-rules"]').getByText("Long-run stop rules", { exact: true }).waitFor();
  await page.locator('[data-budget-guard="schedule-limit"]').getByText("Scheduled work budget", { exact: true }).waitFor();
  await page.locator('[data-budget-guard="evidence-cost"]').getByText("Evidence cost tradeoff", { exact: true }).waitFor();
  await page.locator("#budget-guard-detail").getByRole("heading", { name: "Guardrail", exact: true }).waitFor();
  await page.locator('[data-budget-guard="model-route"]').getByRole("button", { name: "Budget brief" }).click();
  await page.locator("#budget-guard-detail").getByText("Classify simple", { exact: false }).waitFor();
  await page.locator('[data-budget-guard="model-route"]').getByRole("button", { name: "Draft guard" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Budget Guard" }).click();
  await page.locator("#panel-budget.active").waitFor();
  await page.locator('[data-budget-guard="model-route"]').getByRole("button", { name: "Open route" }).click();
  await page.locator("#panel-promptlab.active").waitFor();
  await openManagePanel(page, "Source Registry");
  await page.locator("#panel-sources.active").waitFor();
  await page.locator("#panel-sources").getByRole("heading", { name: "Source Registry" }).waitFor();
  await page.locator(".source-row").nth(4).waitFor();
  await page.locator('[data-source-row="memory"]').getByText("Reviewed memory", { exact: true }).waitFor();
  await page.locator('[data-source-row="sessions"]').getByText("Favorite sessions", { exact: true }).waitFor();
  await page.locator('[data-source-row="runs"]').getByText("Execution evidence", { exact: true }).waitFor();
  await page.locator('[data-source-row="intake"]').getByText("File Intake", { exact: true }).waitFor();
  await page.locator('[data-source-row="council"]').getByText("Council", { exact: true }).waitFor();
  await page.locator("#source-registry-detail").getByRole("heading", { name: "Reliability", exact: true }).waitFor();
  await page.locator('[data-source-row="runs"]').getByRole("button", { name: "Draft maintenance" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.locator("#panel-sources.active").waitFor();
  await page.locator('[data-source-row="council"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-council.active").waitFor();
  await openManagePanel(page, "Knowledge Reconciliation");
  await page.locator("#panel-reconcile.active").waitFor();
  await page.locator("#panel-reconcile").getByRole("heading", { name: "Knowledge Reconciliation" }).waitFor();
  ...
  await page.locator("#knowledge-reconcile-detail").getByRole("heading", { name: "Confidence boundary", exact: true }).waitFor();
  await page.locator('[data-reconcile-risk="weak-citation"]').getByRole("button", { name: "Draft resolution" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Knowledge Reconciliation" }).click();
  await page.locator("#panel-reconcile.active").waitFor();
  await page.locator('[data-reconcile-risk="privacy-boundary"]').getByRole("button", { name: "Open route" }).click();
  ...
  await page.locator("#panel-citation").getByRole("heading", { name: "Citation Planner" }).waitFor();
  await page.getByLabel("Citation claim scope").fill("The release decision is supported by current docs and browser evidence.");
  await page.getByLabel("Citation source posture").fill("official docs plus local memory, fresh browser evidence required");
  await page.getByLabel("Citation evidence level").fill("direct quote or dated source summary with gap report");
  ...
  await page.locator('[data-citation-grounding="coverage"]').getByRole("button", { name: "Draft grounding" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Citation Planner" }).click();
  await page.locator("#panel-citation.active").waitFor();
  await page.locator('[data-citation-grounding="coverage"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-sources.active").waitFor();
  await page.getByRole("button", { name: "Citation Planner" }).click();
  await page.locator("#panel-citation.active").waitFor();
  await page.locator('[data-citation-grounding="quote-capture"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-browser.active").waitFor();
  await page.getByRole("button", { name: "Citation Planner" }).click();
  await page.locator("#panel-citation.active").waitFor();
  await page.locator('[data-citation-grounding="freshness"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-data.active").waitFor();
  await page.getByRole("button", { name: "Citation Planner" }).click();
  await page.locator("#panel-citation.active").waitFor();
  await page.locator('[data-citation-grounding="gap-escalation"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-share.active").waitFor();
  await openManagePanel(page, "Reuse Gallery");
  await page.locator("#panel-reuse.active").waitFor();
  await page.locator("#panel-reuse").getByRole("heading", { name: "Reuse Gallery" }).waitFor();
  ...
  await page.locator('[data-reuse-asset="source-memory"]').getByText("Knowledge", { exact: true }).waitFor();
  await page.locator("#reuse-gallery-detail").getByRole("heading", { name: "Reuse value", exact: true }).waitFor();
  await page.locator('[data-reuse-asset="prompt-evidence"]').getByRole("button", { name: "Draft starter" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Reuse Gallery" }).click();
  await page.locator("#panel-reuse.active").waitFor();
  await page.locator('[data-reuse-asset="source-memory"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-memory.active").waitFor();
  await openManagePanel(page, "Result Library");
  await page.locator("#panel-results.active").waitFor();
  await page.locator("#panel-results").getByRole("heading", { name: "Result Library" }).waitFor();
  await page.locator(".result-library-card").nth(4).waitFor();
  await page.locator(".result-library-card").getByText("Run report", { exact: true }).first().waitFor();
  await page.locator('[data-result-archive="share-result-brief"]').getByText("Handoff pack", { exact: true }).waitFor();
  await page.locator('[data-result-archive="data-result-trend"]').getByText("Insight brief", { exact: true }).waitFor();
  await page.locator('[data-result-archive="citation-result-coverage"]').getByText("Citation brief", { exact: true }).waitFor();
  await page.locator('[data-result-archive="reuse-result-prompt-direct"]').getByText("Reusable output", { exact: true }).waitFor();
  await page.locator("#result-library-detail").getByRole("heading", { name: "Freshness", exact: true }).waitFor();
  await page.locator('[data-result-archive="citation-result-coverage"]').getByRole("button", { name: "Draft follow-up" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Result Library" }).click();
  await page.locator("#panel-results.active").waitFor();
  await page.locator('[data-result-archive="share-result-brief"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-share.active").waitFor();
  await openManagePanel(page, "Playbook Library");
  await page.locator("#panel-playbooks.active").waitFor();
  await page.locator("#panel-playbooks").getByRole("heading", { name: "Playbook Library" }).waitFor();
  await page.locator(".playbook-card").nth(7).waitFor();
  await page.locator('[data-playbook="reviewed-research"]').getByText("Reviewed evidence research", { exact: true }).waitFor();
  await page.locator('[data-playbook="data-insight"]').getByText("Reviewable data insight", { exact: true }).waitFor();
  await page.locator('[data-playbook="handoff-pack"]').getByText("Local handoff package", { exact: true }).waitFor();
  await page.locator('[data-playbook="citation-grounding"]').getByText("Claim grounding review", { exact: true }).waitFor();
  await page.locator('[data-playbook="agent-profile"]').getByText("Focused agent profile", { exact: true }).waitFor();
  await page.locator('[data-playbook="memory-curation"]').getByText("Durable memory curation", { exact: true }).waitFor();
  await page.locator('[data-playbook="delivery-review"]').getByText("Approval-first delivery", { exact: true }).waitFor();
  await page.locator('[data-playbook="council-decision"]').getByText("Multi-role decision review", { exact: true }).waitFor();
  await page.locator("#playbook-library-detail").getByRole("heading", { name: "Evidence gate", exact: true }).waitFor();
  await page.locator('[data-playbook="reviewed-research"]').getByRole("button", { name: "Draft playbook" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.locator("#panel-playbooks.active").waitFor();
  await page.locator('[data-playbook="data-insight"]').getByRole("button", { name: "Open route" }).click();
  await page.locator("#panel-data.active").waitFor();
  await openManagePanel(page, "Starter Kits");
  await page.locator("#panel-starter.active").waitFor();
  await page.locator("#panel-starter").getByRole("heading", { name: "Starter Kits" }).waitFor();
  await page.locator(".starter-kit-card").nth(5).waitFor();
  await page.locator('[data-starter-kit="browser-research"]').getByText("Reviewed web research", { exact: true }).waitFor();
  await page.locator('[data-starter-kit="data-insight"]').getByText("Local data insight brief", { exact: true }).waitFor();
  await page.locator('[data-starter-kit="agent-build"]').getByText("Focused agent profile", { exact: true }).waitFor();
  await page.locator('[data-starter-kit="share-handoff"]').getByText("Local handoff package", { exact: true }).waitFor();
  await page.locator('[data-starter-kit="memory-curation"]').getByText("Durable memory curation", { exact: true }).waitFor();
  await page.locator('[data-starter-kit="reuse-polish"]').getByText("Reusable workflow polish", { exact: true }).waitFor();
  await page.locator("#starter-kit-detail").getByRole("heading", { name: "Safety", exact: true }).waitFor();
  await page.locator('[data-starter-kit="browser-research"]').getByRole("button", { name: "Draft kit" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Starter Kits" }).click();
  await page.locator("#panel-starter.active").waitFor();
  await page.locator('[data-starter-kit="browser-research"]').getByRole("button", { name: "Open route" }).click();
  await page.locator("#panel-browser.active").waitFor();
  await page.getByRole("button", { name: "Starter Kits" }).click();
  await page.locator("#panel-starter.active").waitFor();
  await page.locator('[data-starter-kit="data-insight"]').getByRole("button", { name: "Open route" }).click();
  await page.locator("#panel-data.active").waitFor();
  await page.getByRole("button", { name: "Starter Kits" }).click();
  await page.locator("#panel-starter.active").waitFor();
  await page.locator('[data-starter-kit="share-handoff"]').getByRole("button", { name: "Open route" }).click();
  await page.locator("#panel-share.active").waitFor();
  await page.getByRole("button", { name: "Starter Kits" }).click();
  await page.locator("#panel-starter.active").waitFor();
  await page.locator('[data-starter-kit="memory-curation"]').getByRole("button", { name: "Open route" }).click();
  await page.locator("#panel-memory.active").waitFor();
  await openManagePanel(page, "Share Pack");
  await page.locator("#panel-share.active").waitFor();
  await page.locator("#panel-share").getByRole("heading", { name: "Share Pack" }).waitFor();
  await page.getByLabel("Share package name").fill("release research handoff");
  await page.getByLabel("Share package audience").fill("future reviewer");
  await page.getByLabel("Share package intent").fill("Reuse verified evidence and continue the release decision.");
  await page.locator(".share-pack-card").nth(4).waitFor();
  await page.locator('[data-share-pack="brief"]').getByText("Executive handoff brief", { exact: true }).waitFor();
  await page.locator('[data-share-pack="evidence"]').getByText("Evidence bundle checklist", { exact: true }).waitFor();
  await page.locator('[data-share-pack="prompt"]').getByText("Reusable prompt starter", { exact: true }).waitFor();
  await page.locator('[data-share-pack="knowledge"]').getByText("Memory handoff notes", { exact: true }).waitFor();
  await page.locator('[data-share-pack="review"]').getByText("Reviewer acceptance checklist", { exact: true }).waitFor();
  await page.locator("#share-pack-detail").getByRole("heading", { name: "Boundary", exact: true }).waitFor();
  await page.locator('[data-share-pack="brief"]').getByRole("button", { name: "Draft pack" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.locator("#panel-share.active").waitFor();
  await page.locator('[data-share-pack="prompt"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-reuse.active").waitFor();
  await page.getByRole("button", { name: "Share Pack" }).click();
  await page.locator("#panel-share.active").waitFor();
  await page.locator('[data-share-pack="knowledge"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-memory.active").waitFor();
  await openManagePanel(page, "Workspace Snapshot");
  await page.locator("#panel-snapshot.active").waitFor();
  await page.locator("#panel-snapshot").getByRole("heading", { name: "Workspace Snapshot" }).waitFor();
  await page.locator(".workspace-snapshot-card").nth(6).waitFor();
  await page.locator('[data-workspace-snapshot="resume"]').getByText("Session resume snapshot", { exact: true }).waitFor();
  await page.locator('[data-workspace-snapshot="evidence"]').getByText("Run evidence snapshot", { exact: true }).waitFor();
  await page.locator('[data-workspace-snapshot="memory-source"]').getByText("Memory and source snapshot", { exact: true }).waitFor();
  await page.locator('[data-workspace-snapshot="result-archive"]').getByText("Result archive snapshot", { exact: true }).waitFor();
  await page.locator('[data-workspace-snapshot="playbook-reuse"]').getByText("Playbook and reuse snapshot", { exact: true }).waitFor();
  await page.locator('[data-workspace-snapshot="delivery-schedule"]').getByText("Delivery and schedule snapshot", { exact: true }).waitFor();
  await page.locator('[data-workspace-snapshot="privacy"]').getByText("Redaction and handoff boundary", { exact: true }).waitFor();
  await page.locator("#workspace-snapshot-detail").getByRole("heading", { name: "Included context", exact: true }).waitFor();
  await page.locator("#workspace-snapshot-detail").getByRole("heading", { name: "Privacy boundary", exact: true }).waitFor();
  await page.locator('[data-workspace-snapshot="privacy"]').getByRole("button", { name: "Snapshot brief" }).click();
  await page.locator("#workspace-snapshot-detail").getByText("Default to local-only", { exact: false }).waitFor();
  await page.locator('[data-workspace-snapshot="privacy"]').getByRole("button", { name: "Draft snapshot" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Workspace Snapshot" }).click();
  await page.locator("#panel-snapshot.active").waitFor();
  await page.locator('[data-workspace-snapshot="result-archive"]').getByRole("button", { name: "Open route" }).click();
  await page.locator("#panel-results.active").waitFor();
  await openManagePanel(page, "Browser Planner");
  await page.locator("#panel-browser.active").waitFor();
  await page.locator("#panel-browser").getByRole("heading", { name: "Browser Planner" }).waitFor();
  await page.getByLabel("Browser target URL").fill("https://example.com/research");
  await page.getByLabel("Browser mission goal").fill("Verify the release notes and capture visual evidence.");
  await page.locator(".browser-mission-card").nth(4).waitFor();
  await page.locator('[data-browser-mission="inspect"]').getByText("Inspect", { exact: true }).waitFor();
  await page.locator('[data-browser-mission="screenshot"]').getByText("Screenshot", { exact: true }).waitFor();
  await page.locator('[data-browser-mission="extract"]').getByText("Extract", { exact: true }).waitFor();
  await page.locator('[data-browser-mission="form-check"]').getByText("Form check", { exact: true }).waitFor();
  await page.locator('[data-browser-mission="monitor"]').getByText("Monitor", { exact: true }).waitFor();
  await page.locator("#browser-mission-detail").getByRole("heading", { name: "Risk", exact: true }).waitFor();
  await page.locator('[data-browser-mission="inspect"]').getByRole("button", { name: "Draft mission" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.locator("#panel-browser.active").waitFor();
  await page.locator('[data-browser-mission="monitor"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-schedules.active").waitFor();
  await openManagePanel(page, "Data Planner");
  await page.locator("#panel-data.active").waitFor();
  await page.locator("#panel-data").getByRole("heading", { name: "Data Planner" }).waitFor();
  await page.getByLabel("Data source descriptor").fill("revenue_metrics.csv with weekly plan and actual columns");
  await page.getByLabel("Data analysis question").fill("Which segments are drifting from plan and need review?");
  await page.getByLabel("Data output format").fill("ranked findings with chart brief and reusable memory");
  await page.locator(".data-insight-card").nth(4).waitFor();
  await page.locator('[data-data-insight="profile"]').getByText("Profile", { exact: true }).waitFor();
  await page.locator('[data-data-insight="trend"]').getByText("Trend", { exact: true }).waitFor();
  await page.locator('[data-data-insight="anomaly"]').getByText("Anomaly", { exact: true }).waitFor();
  await page.locator('[data-data-insight="visual"]').getByText("Chart brief", { exact: true }).waitFor();
  await page.locator('[data-data-insight="knowledge"]').getByText("Knowledge", { exact: true }).waitFor();
  await page.locator("#data-insight-detail").getByRole("heading", { name: "Guardrail", exact: true }).waitFor();
  await page.locator('[data-data-insight="trend"]').getByRole("button", { name: "Draft analysis" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Data Planner" }).click();
  await page.locator("#panel-data.active").waitFor();
  await page.locator('[data-data-insight="visual"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-reuse.active").waitFor();
  await page.getByRole("button", { name: "Data Planner" }).click();
  await page.locator("#panel-data.active").waitFor();
  await page.locator('[data-data-insight="knowledge"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-memory.active").waitFor();
  await page.getByRole("button", { name: "Home" }).click();
  await page.locator("#panel-home.active").waitFor();
  ...
  await page.getByRole("heading", { name: "Proactive Delivery" }).waitFor();
  ...
  await page.locator("#delivery-detail").getByRole("heading", { name: "Evidence", exact: true }).waitFor();
  await page.locator('[data-delivery-lane="scheduled-work"]').getByRole("button", { name: "Draft delivery" }).click();
  await page.locator("#panel-chat.active").waitFor();
  ...
  await page.getByRole("button", { name: "Delivery" }).click();
  await page.locator("#panel-delivery.active").waitFor();
  await page.locator('[data-delivery-lane="channel-readiness"]').getByRole("button", { name: "Open source" }).click();
  await page.locator("#panel-inbox.active").waitFor();
  await openManagePanel(page, "Schedules");
