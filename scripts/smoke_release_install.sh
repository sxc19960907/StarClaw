...
BASE_URL="${RELEASE_SMOKE_BASE_URL:-http://127.0.0.1:7533}"
TMP_DIR="$(mktemp -d)"
...
if [[ -z "$ARCHIVE" ]]; then
  fail "set RELEASE_ARCHIVE=/path/to/starclaw_<OS>_<ARCH>.tar.gz or pass the archive path as the first argument"
fi
[[ -f "$ARCHIVE" ]] || fail "archive not found: $ARCHIVE"

require_cmd curl
mkdir -p "$INSTALL_DIR" "$SMOKE_HOME/.starclaw"
...
BIN="$(find "$INSTALL_DIR" -type f \( -name starclaw -o -name starclaw.exe \) | sort | head -n 1)"
[[ -n "$BIN" ]] || fail "starclaw executable not found in archive"
if [[ "$BIN" != *.exe ]]; then
  chmod +x "$BIN"
fi
...
env HOME="$SMOKE_HOME" "$BIN" version > "$TMP_DIR/version.out"
...
expect_contains app-check "Health:        $BASE_URL/health"
expect_contains app-check "Status API:    $BASE_URL/status"
